...
void VulkanDeviceManager::SetupDebugMessenger() {
    ...
    if (VulkanDeviceManager::CreateDebugUtilsMessengerEXT(m_Instance, &create_info, nullptr, &m_DebugMessenger) != VK_SUCCESS) {
...
void VulkanDeviceManager::CreateSyncObjects() {
...
void VulkanDeviceManager::DrawFrame() {
...
void VulkanDeviceManager::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& create_info) {
    create_info                 = {};
...
bool VulkanDeviceManager::checkValidationLayerSupport() {
...
uint32_t VulkanDeviceManager::findMemoryType(uint32_t type_filter, VkMemoryPropertyFlags properties) {
...
void VulkanDeviceManager::recordCommandBuffer(VkCommandBuffer command_buffer, uint32_t image_index) {
    ...
    for (const auto& game_object : p_RenderMesh->getGameObjects()) {
        ...
        vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, p_PipelineManager->getPipelineLayout(), 0, 1, &game_object.descriptor_sets[m_CurrentFrame], 0, nullptr);
...
void VulkanDeviceManager::updateUniformBuffer(uint32_t current_image) {
...
QueueFamilyIndices VulkanDeviceManager::findQueueFamilies(VkPhysicalDevice device, VkSurfaceKHR surface) {
...
void VulkanDeviceManager::createImage(uint32_t width, uint32_t height, uint32_t mip_levels, VkSampleCountFlagBits num_samples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& image_memory) {
...
VkImageView VulkanDeviceManager::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspect_flags, uint32_t mip_levels) {
...
VkFormat VulkanDeviceManager::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
VkFormat VulkanDeviceManager::findDepthFormat() {
...
void VulkanDeviceManager::endSingleTimeCommands(VkCommandBuffer command_buffer) {
...
void VulkanDeviceManager::copyBuffer(VkBuffer src_buffer, VkBuffer dst_buffer, VkDeviceSize size) {
...
void VulkanDeviceManager::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& buffer_memory) {
...
void VulkanDeviceManager::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout old_layout, VkImageLayout new_layout, uint32_t mip_levels) {
    VkCommandBuffer command_buffer = beginSingleTimeCommands();
...
void VulkanDeviceManager::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer command_buffer = beginSingleTimeCommands();
...
VkResult VulkanDeviceManager::CreateDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT* p_create_info, const VkAllocationCallbacks* p_allocator, VkDebugUtilsMessengerEXT* p_debug_messenger) {
...
void VulkanDeviceManager::DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debug_messenger, const VkAllocationCallbacks* p_allocator) {
